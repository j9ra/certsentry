...
public class SecUserAppRepositoryApiKeyUserDetailsService implements ApiKeyUserDetailsService {
	...
	public UserDetails loadUserByApiKey(String apiKey) throws UsernameNotFoundException {
		...
		Optional<SecUser> secUser = secUserAppDataService.getUserByApiKey(apiKey);
		if(secUser.isPresent()) {
			return new SimpleUserDetails(secUser.get());
		}
		
		throw new UsernameNotFoundException("User with ApiKey [" + apiKey + "] not found");
	}
